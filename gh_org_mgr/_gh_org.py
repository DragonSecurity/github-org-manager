...
class GHorg:  # pylint: disable=too-many-instance-attributes, too-many-lines
    ...
    def login(
        ...
        if self.gh_app_id and self.gh_app_private_key:
            ...
            logging.debug("Using app id %s", self.gh_app_id)
            ...
            try:
                ...
                logging.critical("Using app id %s", self.gh_app_id)
                logging.critical("Using app private key %s", self.gh_app_private_key)
    ...
    def _create_perms_changelist_for_teams(
        ...
        existing_repo_names = {repo.name: repo for repo in self.org.get_repos()}
        ...
        for team_name, team_attrs in self.configured_teams.items():
            ...
            try:
                ...
                logging.debug("Team '%s' not found (likely dry-run)", team_name)
                continue
